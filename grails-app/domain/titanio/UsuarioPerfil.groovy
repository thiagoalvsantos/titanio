...
class UsuarioPerfil implements Serializable {
	...
	static UsuarioPerfil create(Usuario usuario, Perfil perfil, Projeto projeto,boolean flush = false) {
		new UsuarioPerfil(usuario: usuario, perfil: perfil, projeto:projeto).save(flush: flush, insert: true)
	}
	...
	static mapping = {
		id composite: ['perfil', 'usuario', 'projeto']
		version false
	}
}
