		...
		<div class="block-border">
			<form class="block-content form" id="table_form" method="post"
				...
				<table class="table sortable no-margin" cellspacing="0" width="100%">
					...
					<%      } else { %>
						<th  scope="col"  title="\${message(code: '${domainClass.propertyName}.${p.name}.label', default: '${p.naturalName}')}">
							<span class="column-sort">
								<a href="#" title="Sort up" class="sort-up"></a>
								<a href="#" title="Sort down" class="sort-down"></a>
							</span><g:message code="${domainClass.propertyName}.${p.name}.label" default="${p.naturalName}" />
						</th>
					<%  }   }   } %>
						</tr>
					</thead>
